...
function gravatar($email, $event) {
  return 'https://2020.indieweb.org/img.php?event='.$event.'&img='.md5(strtolower(trim($email))).'.jpg';
}
...
function get_active_events() {
  return json_decode(file_get_contents(__DIR__.'/../active-events.json'), true);
}
